...
# Updated with AmneziaWG 2.0 support
...
# Function to read I1-I5 parameters
function readIParams() {
    # Default value for I1
    ...
    echo -e "\n${GREEN}I1-I5 Parameters (optional):${NC}"
    echo "These are optional parameters for additional configuration."
    
    # I1 with default value
    read -rp "I1 [default: <b 0x0844...>]: " SERVER_AWG_I1
    if [[ -z "${SERVER_AWG_I1}" ]]; then
        SERVER_AWG_I1="${DEFAULT_I1}"
    fi
    
    # I2 (optional)
    read -rp "I2 (optional, press Enter to skip): " SERVER_AWG_I2
    
    # I3 (optional)
    read -rp "I3 (optional, press Enter to skip): " SERVER_AWG_I3
    
    # I4 (optional)
    read -rp "I4 (optional, press Enter to skip): " SERVER_AWG_I4
    
    # I5 (optional)
    read -rp "I5 (optional, press Enter to skip): " SERVER_AWG_I5
}
...
# Function to change I parameters - ИСПРАВЛЕННАЯ ВЕРСИЯ
    ...
    while true; do
        ...
        case "${CHOICE}" in
            1)
                ...
                echo -n "Enter new I1 value (or press Enter to keep current): "
                read NEW_VALUE
                if [[ -n "${NEW_VALUE}" ]]; then
            ...
            2)
                ...
                echo -n "Enter new I2 value (or press Enter to keep current, 'none' to remove): "
                read NEW_VALUE
            ...
            3)
                ...
                echo -n "Enter new I3 value (or press Enter to keep current, 'none' to remove): "
                read NEW_VALUE
                if [[ "${NEW_VALUE}" == "none" ]]; then
                    SERVER_AWG_I3=""
            ...
            4)
                ...
                echo -n "Enter new I4 value (or press Enter to keep current, 'none' to remove): "
                read NEW_VALUE
                if [[ "${NEW_VALUE}" == "none" ]]; then
                    SERVER_AWG_I4=""
            ...
            5)
                ...
                echo -n "Enter new I5 value (or press Enter to keep current, 'none' to remove): "
                read NEW_VALUE
                if [[ "${NEW_VALUE}" == "none" ]]; then
                    SERVER_AWG_I5=""
...
SERVER_AWG_I5=$(safeQuoteParam "${SERVER_AWG_I5}")" >"${AMNEZIAWG_DIR}/params"
...
H4 = ${SERVER_AWG_H4}" >"${SERVER_AWG_CONF}"
    ...
    # Add I parameters to server config (only if they have values)
    {
...
H2 = ${SERVER_AWG_H2}
H3 = ${SERVER_AWG_H3}
H4 = ${SERVER_AWG_H4}" >"${HOME_DIR}/${SERVER_AWG_NIC}-client-${CLIENT_NAME}.conf"
...
function loadParams() {
    ...
    *)
    ...
    if ! cp "${SERVER_AWG_CONF}.bak" "${SERVER_AWG_CONF}" 2>/dev/null; then
        ...
        if [[ -f "${AMNEZIAWG_DIR}/params.bak" ]]; then
...
SERVER_AWG_I3=$(safeQuoteParam "${SERVER_AWG_I3}")
SERVER_AWG_I4=$(safeQuoteParam "${SERVER_AWG_I4}")
SERVER_AWG_I5=$(safeQuoteParam "${SERVER_AWG_I5}")" >"${PARAMS_TMP}"; then
        rm -f "${PARAMS_TMP}"
    ...
    if systemctl is-active --quiet "awg-quick@${SERVER_AWG_NIC}"; then
        ...
        systemctl restart "awg-quick@${SERVER_AWG_NIC}"
        echo -e "${GREEN}AmneziaWG configuration reloaded.${NC}"
    fi
...
function showClientQR() {
    ...
    if [[ -f "${HOME_DIR}/${SERVER_AWG_NIC}-client-${CLIENT_NAME}.conf" ]]; then
        ...
        echo -e "${GREEN}Config file: ${HOME_DIR}/${SERVER_AWG_NIC}-client-${CLIENT_NAME}.conf${NC}"
        echo -e "${GREEN}========================================${NC}"
        echo ""
...
# ИСПРАВЛЕННАЯ ФУНКЦИЯ manageMenu
function manageMenu() {
    while true; do
        echo "AmneziaWG 2.0 server installer (https://github.com/potap1978/amneziawg-install_awg_v2.0)"
        echo ""
        ...
        echo "   5) Change I1-I5 parameters"
        echo "   6) Uninstall AmneziaWG"
        echo "   7) Backup settings"
        echo "   8) Restore settings"
        echo "   9) Exit"
        
        local MENU_OPTION=""
        local MENU_PROMPT="Select an option [1-9]: "
        ...
        while true; do
            ...
            if [[ "$KEY" =~ [1-9] ]]; then
                ...
                # Если не цифра от 1 до 9
                echo -e "\r\033[K${MENU_PROMPT}Please enter a number between 1 and 9.\n$MENU_PROMPT"
            fi
        done
        ...
        5)
            changeIParams
            # После возврата из changeIParams продолжаем цикл меню
            ;;
        6)
            uninstallAmneziaWG
            # После удаления проверяем, существует ли еще установка
            if [[ ! -e "${AMNEZIAWG_DIR}/params" ]]; then
                echo -e "${GREEN}AmneziaWG has been uninstalled. Exiting.${NC}"
                exit 0
            fi
            ;;
        7)
